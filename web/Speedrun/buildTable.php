<?php
    ...
    if($category!='category.id'){
        ...
        {
            echo '<tr><td>'.$row['username'].'</td><td>'.formatTime($row['time']).'</td><td>'.$row['name'].'</td><td>'.valitity($row['valid']).'</td></tr>';
       }
    }
    else{
        ...
        echo '<tr><th>User</th><th>Time</th><th>Category</th><th>Platform</th><th style="text-align:center">Validity</th></tr>';
        while ($row = $statement->fetch(PDO::FETCH_ASSOC))
        {
            echo '<tr><td>'.$row['username'].'</td><td>'.formatTime($row['time']).'</td><td>'. $row['category_title'] .'</td><td>'.$row['name'].'</td><td>'.valitity($row['valid']).'</td></tr>';
        }
    }
